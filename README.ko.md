...
Claude가 세션 중 호출할 수 있는 8개의 기억 도구를 제공합니다:
...
**memrosetta_feedback** -- 검색 결과 피드백(good/bad)을 기록해 랭킹을 튜닝합니다.

**memrosetta_reconstruct_recall** (v0.10+) -- 해마 레이어 기반 재구성 회상. 원시 청크가 아닌 저장된 binding 으로부터 재조립된 episode + gist 를 반환합니다.
...
3. LLM 추출기(Claude Haiku → GPT-4o-mini → 규칙 기반 폴백 → 없음)를
...
엔진은 저장, 검색, 관계 확장, 망각을 처리합니다 -- 모두 로컬에서, API 호출 없이. AI가 무엇을 저장할지 결정합니다. MemRosetta는 어떻게 저장하고 검색할지 결정합니다.

### 원자적 기억 + FTS5 + 활성화 기반 랭킹

MemRosetta는 **원자적 기억**(텍스트 덩어리가 아닌 사실 하나 = 레코드 하나)을 로컬 SQLite에 저장합니다. 조회는 SQLite FTS5 (BM25) 키워드 검색에 활성화 점수, 최근성, 관계 그래프 인접성, Hebbian 동시활성 엣지 가중치를 결합합니다.

```
쿼리: "CSS 프레임워크 뭘로 정했지?"
  |
  +-- FTS5 (BM25)           키워드 + 내용 매칭
  +-- Activation 부스트      자주 접근한 기억이 우선
  +-- Recency 부스트         최신 기억 우선 (시간 감쇠 0.99/hr)
  +-- 관계 확장              top 히트의 그래프 인접 기억까지 확장
  +-- Hebbian co-access      함께 회상되는 기억은 함께 가중
```

벡터 / 임베딩 기반 시맨틱 검색은 **v0.11에서 제거**되었습니다. Hugging Face
의존성(~1.5GB)을 없애고 30MB 이하 설치로 전환했습니다. 현재는 FTS5 +
활성화 + 관계 그래프 + Hebbian co-access가 검색의 전부입니다.
...
저장                        검색                        유지보수
----                        ----                        ------
분류 (사실/선호/             FTS5 BM25                   활성화 점수 계산
  결정/이벤트)               활성화 가중치                 (ACT-R 모델)
원자적 저장                  Recency 부스트               계층 압축
autoRelate로 연결            관계 확장                      Hot  -> 항상 로드
관계 연결                   확산 활성화                    Warm -> 최근 30일
                           Co-access 가중                 Cold -> 압축 저장
```
...
| **검색** | 벡터 유사도만 | FTS5 + 활성화 + 관계 그래프 + co-access |
| **시간** | 없음 | 기억당 4개 타임스탬프 |
| **망각** | 모두 동일 가중치 | ACT-R: 자주 쓰면 높은 순위 |

## 재구성 회상 (Reconstructive Recall, v0.10+)

검색과 별개로, v0.10은 해마(hippocampal)의 패턴 분리/완성을 모델링한
재구성 회상 레이어를 추가했습니다. `memrosetta recall` (CLI)과
`memrosetta_reconstruct_recall` (MCP)은 원시 텍스트 청크가 아니라 저장된
binding 으로부터 재조립된 episode + gist 를 반환합니다. 전체 스펙은
[docs/reconstructive-memory-spec.md](docs/reconstructive-memory-spec.md)
참고.
...
**검색** -- SQLite FTS5 (BM25) 키워드 + 내용 검색에 활성화 점수, 최근성 감쇠, 관계 그래프 확장, Hebbian 동시활성, 그래프 확산 활성화 가중을 결합.

**재구성 회상 (v0.10+)** -- `memrosetta recall` 과 `memrosetta_reconstruct_recall` MCP 도구가 원시 청크가 아닌 해마 레이어의 binding 으로부터 재조립된 episode + gist 를 반환. 인간 패턴 분리 / 완성을 모델링.
...
**100% 로컬, ML 의존성 제로 (v0.11+).** 설치 용량이 ~1.5GB → ~30MB. Hugging Face 없음, sqlite-vec 없음, 로컬 추론 없음. FTS5 + 활성화 + 재구성 회상이 전부.

**950개 이상의 테스트, 75개 테스트 파일.**
...
| `memrosetta_feedback` | 검색 결과 피드백(good/bad) 기록 — 랭킹 튜닝 |
| `memrosetta_reconstruct_recall` | 재구성 회상 (v0.10+): 해마 binding 으로부터 episode + gist 재조립 |
...
| 패키지 | 설명 |
|--------|------|
| `@memrosetta/core` | 메모리 엔진: SQLite + FTS5 + 관계 그래프 + 재구성 회상. LLM 의존성 없음, ML 의존성 없음. |
| `@memrosetta/cli` | CLI (`recall`, `search`, `sync` 포함 22개 명령) |
| `@memrosetta/mcp` | MCP 서버 (AI 도구 연동, 8개 도구) |
...
| **FTS + Fact Extraction** | **0.0311** | **0.0572** | **4.0ms** | **Yes (외부)** |

LoCoMo 대화 턴 데이터에서 FTS5 키워드 매칭이 LLM 없는 베이스라인. 사실 추출(원자적 메모리 전처리)이 **single-hop 23.8%**로 최고 정확도.

사실 추출은 외부 LLM(OpenAI, Anthropic 등)을 사용하여 대화 트랜스크립트를 원자적 사실로 전처리합니다. 코어 검색 엔진은 LLM 없이 동작합니다.

> 벡터+FTS 하이브리드 벤치마크는 v0.11에서 Hugging Face 임베딩 제거와 함께
> 중단되었습니다. 이전 수치는 git history에 보존되어 있습니다.

```bash
pnpm bench:sqlite                    # FTS only
pnpm bench:sqlite --converter fact --llm-provider openai  # LLM 추출 포함
```
...
| 재구성 회상 | No | No | No | **Yes (해마 모델, v0.10+)** |
...
pnpm test              # 950개 이상의 테스트, 75개 테스트 파일
...
- [x] 재구성 메모리 커널 — Layer A + Layer B (v0.10.0)
- [x] `memrosetta recall` + MCP `reconstruct_recall` + v1.0 벤치마크 스위트 (v0.10.0)
