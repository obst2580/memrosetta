...
export function NotAnotherRag() {
  return (
    <Section id="not-rag" className="border-t border-zinc-900">
        ...
        <table className="w-full text-left text-sm">
          <thead>
            <tr className="border-b border-zinc-800">
              <th className="pb-3 pr-6 font-medium text-zinc-500" />
              <th className="pb-3 pr-6 font-mono text-xs font-medium text-zinc-500">
                RAG (chunk-based)
              </th>
          ...
          <tbody>
            {COMPARISON_ROWS.map((row, i) => (
              <tr key={i} className="border-b border-zinc-800/50">
                ...
                <td className="py-3 pr-6 text-zinc-500">{row.rag}</td>
                <td className="py-3 text-zinc-300">{row.memrosetta}</td>
              </tr>
            ))}
          </tbody>
        </table>
      ...
      <div className="mt-12 grid gap-6 md:grid-cols-2">
        ...
        <div className="rounded-lg border border-zinc-800 bg-zinc-900/30 p-6">
          <h3 className="mb-4 font-mono text-sm font-medium text-zinc-500">
            RAG approach
          </h3>
          <div className="space-y-2">
            {['Chunk 1: "...API uses Spring Boot with Post..."', 'Chunk 2: "...greSql, deployed on Azure. We al..."', 'Chunk 3: "...so decided to use approach B fo..."'].map(
              (chunk, i) => (
                <div
                  key={i}
                  className="rounded border border-zinc-800 bg-zinc-950 px-3 py-2 font-mono text-xs text-zinc-600"
                >
                  {chunk}
                </div>
              ),
        ...
        <div className="rounded-lg border border-amber-500/20 bg-amber-500/5 p-6">
          ...
          <div className="space-y-2">
            {[
              ...
              <div
                key={i}
                className="flex items-start gap-2 rounded border border-amber-500/10 bg-zinc-950 px-3 py-2"
              >
                <span className="mt-px rounded bg-amber-500/10 px-1.5 py-0.5 font-mono text-[10px] text-amber-400">
                  {memory.type}
                </span>
